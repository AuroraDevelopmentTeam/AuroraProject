...
class MyEmbedDescriptionPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        embed = Embed(title="Shop", description="\n".join(entries), color=DEFAULT_BOT_COLOR)
        embed.set_footer(text=f'{menu.current_page + 1}/{self.get_max_pages()}')
        return embed


class SelectButtonMenuPages(menus.ButtonMenuPages, inherit_buttons=False):
    def __init__(self, source: menus.PageSource, guild: nextcord.Guild, disabled: bool, timeout: int = 60):
        self.guild = guild
        super().__init__(source, timeout=timeout, disable_buttons_after=True, style=nextcord.ButtonStyle.secondary)
