...
def create_goodbye_config() -> None:
    ...
    cursor = db.cursor()
    cursor.execute(f"""CREATE TABLE IF NOT EXISTS goodbye_config ( guild_id INTERGER, goodbye_message_enabled BOOLEAN, 
...
def create_server_goodbye_embed(member: typing.Union[nextcord.Member, nextcord.User],
                                guild: nextcord.Guild) -> nextcord.Embed:
    db = sqlite3.connect("./databases/main.sqlite")
    cursor = db.cursor()
    ...
    if "{member.id}" in goodbye_message_title:
        goodbye_message_title = goodbye_message_title.replace("{member.id}", f"{member.id}")
    if "{guild.id}" in goodbye_message_title:
        goodbye_message_title = goodbye_message_title.replace("{guild.id}", f"{guild.id}")
    if "{guild.name}" in goodbye_message_title:
        goodbye_message_title = goodbye_message_title.replace("{guild.name}", f"{guild.name}")
    if "{guild.id}" in goodbye_message_title:
        goodbye_message_title = goodbye_message_title.replace("{guild.id}", f"{guild.id}")
    if "{guild.members}" in goodbye_message_title:
        goodbye_message_title = goodbye_message_title.replace("{guild.members}", f"{len(guild.members)}")
    if "{member.mention}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{member.mention}", f"{member.mention}")
    if "{member.tag}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{member.tag}", f"{member.tag}")
    if "{member.created_at}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{member.created_at}", f"{member.created_at}")
    if "{member.name}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{member.name}", f"{member.name}")
    if "{member.id}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{member.id}", f"{member.id}")
    if "{guild.id}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{guild.id}", f"{guild.id}")
    if "{guild.name}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{guild.name}", f"{guild.name}")
    if "{guild.id}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{guild.id}", f"{guild.id}")
    if "{guild.members}" in goodbye_message_description:
        goodbye_message_description = goodbye_message_description.replace("{guild.members}", f"{len(guild.members)}")
    list_of_channels = [t for t in goodbye_message_description.split() if t.startswith('#')]
