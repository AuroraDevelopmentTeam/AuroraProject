        ...
        for member in guild.members:
            if not member.bot:
                if is_user_in_table("marriage", guild.id, member.id) is False:
                    sql = "INSERT INTO marriage(guild_id, user_id, pair_id, like_id, divorces, love_description, " \
        ...
        for member in guild.members:
            if not member.bot:
                if is_user_in_table("gifts", guild.id, member.id) is False:
                    sql = "INSERT INTO gifts(guild_id, user_id, gift_1, gift_2, gift_3, gift_4, gift_5, gift_6, " \
