...
def get_user_love_description(guild_id: int, user_id: int) -> str:
    ...
    love_description = \
        cursor.execute(
            f"SELECT love_description FROM marriage WHERE guild_id = {guild_id} AND user_id = {user_id}").fetchone()[0]
    cursor.close()
    db.close()
...
def get_user_marry_date(guild_id: int, user_id: int) -> str:
    ...
    cursor = db.cursor()
    date = \
        cursor.execute(f"SELECT date FROM marriage WHERE guild_id = {guild_id} AND user_id = {user_id}").fetchone()[0]
    cursor.close()
    db.close()
    return date
