...
def construct_log(event_type: str, guild: nextcord.Guild, message: nextcord.Message = None, jump_url: bool = False,
                  display_message: bool = True, message_channel: nextcord.TextChannel = None, user: nextcord.Member =
                  None, before: nextcord.Message = None, after: nextcord.Message = None, role: nextcord.Role = None,
                  role_before: nextcord.Role = None, role_after: nextcord.Role = None,
                  channel=None, channel_before=None, channel_after=None) -> nextcord.Embed:
    ...
    if jump_url is True:
        embed.add_field(name="URL", value=f"[message]({message.jump_url})", inline=False)
    ...
    if role is not None:
        embed.add_field(name="Role", value=f"{role.mention}\n"
                                           ...
                                           f"Permissions: **{role.permissions.value}**", inline=False)
    if role_before is not None:
        embed.add_field(name="Before", value=f"``@{role_before.name}``, ``{role_before.color}``\n"
                                             f"Position: **{role_before.position}**, mentionable: "
                                             f"**{role_before.mentionable}**\n"
                                             f"Permissions: **{role_before.permissions.value}**", inline=True)
    if role_after is not None:
        embed.add_field(name="After", value=f"{role_after.mention}\n``@{role_after.name}``, ``{role_after.color}``\n"
                                            f"Position: **{role_after.position}**, mentionable: "
                                            f"**{role_after.mentionable}**\n"
                                            f"Permissions: **{role_after.permissions.value}**", inline=True)
    ...
    if channel_after is not None:
        embed.add_field(name="After", value=f"``{channel_after.name}``, {channel_after.mention}\n"
                                            f"Changed roles: {channel_after.changed_roles} ", inline=True)
