...
def get_phrase(player_choice: str, computer_choice: str) -> str:
    phrases = {
        ...
        "en_us": {
            ...
            ("brick", "knife"): "Brick and knife. Uhm.. Hey, Knife, can you at least try? Seems no",
            ("brick", "evidence"): "Brick and evidence. Just like paper and rock",
            ("brick", "yandere"): "Brick and yandere. New weapon for yandere",
            ("brick", "tentacles"): "Brick and tentacles. Tentacles can't do anything to a brick!?",
            ("knife", "brick"): "Brick and knife. Uhm.. Hey, Knife, can you at least try? Seems no",
            ("knife", "knife"): "Knife and knife. CUT-CUT-CUT",
            ("knife", "evidence"): "Knife and evidence. Like paper and scissors",
            ("knife", "yandere"): "Knife and yandere. Knife is yandere's favorite weapon",
            ("knife", "tentacles"): "Knife and tentacles. Dear, we're having seafood for dinner tonight",
            ("evidence", "brick"): "Brick and evidence. Just like paper and rock",
            ("evidence", "knife"): "Knife and evidence. Like paper and scissors",
            ("evidence", "evidence"): "Evidence and evidence. Now yandere really don't stand a chance",
            ("evidence", "yandere"): "Evidence and yandere. Now her sempai know truth",
            ("evidence", "tentacles"): "Evidence and tentacles. Now this is not evidence, but just a paper ball",
            ("yandere", "brick"): "Brick and yandere. New weapon for yandere",
            ("yandere", "knife"): "Knife and yandere. Knife is yandere's favorite weapon",
            ...
            ("tentacles", "brick"): "Brick and tentacles. Tentacles can't do anything to a brick!?",
            ("tentacles", "knife"): "Knife and tentacles. Dear, we're having seafood for dinner tonight",
            ("tentacles", "evidence"): "Evidence and tentacles. Now this is not evidence, but just a paper ball",
            ("tentacles", "yandere"): "Yandere and tentacles. Is this new hentai?",
            ("tentacles", "tentacles"): "Tentacles on tentacles, tentacles on tentacles and tentacles in tentacles at "
...
def create_starting_embed(title, description) -> nextcord.Embed:
    embed = nextcord.Embed(title=title, description=description, color=DEFAULT_BOT_COLOR)
    return embed
...
async def brick_button_callback(interaction: Interaction):
    ...
    if is_win is True:
        embed = create_final_embed("Brick Knife Evidence Yandere Tentacles", f"**Player** win")
        embed.add_field(name='Your choice:', value=f'{emojify_choice(player_choice)}', inline=True)
    ...
    if is_win is None:
        embed = create_final_embed("Brick Knife Evidence Yandere Tentacles", f"**Draw**")
        embed.add_field(name='Your choice:', value=f'{emojify_choice(player_choice)}', inline=True)
        embed.add_field(name='My choice:', value=f'{emojify_choice(computer_choice)}')
        embed.set_footer(text=phrase)
        view = create_final_view()
        await interaction.message.edit(embed=embed, view=view)
    if is_win is False:
        embed = create_final_embed("Brick Knife Evidence Yandere Tentacles", f"**Player** loose")
        embed.add_field(name='Your choice:', value=f'{emojify_choice(player_choice)}', inline=True)
        embed.add_field(name='My choice:', value=f'{emojify_choice(computer_choice)}')
        embed.set_footer(text=phrase)
        view = create_final_view()
