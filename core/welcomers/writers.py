...
def write_in_welcomers_config_standart_values(guilds) -> None:
    ...
    for guild in guilds:
        if is_guild_id_in_table("welcomers_config", guild.id) is False:
            ...
            sql = "INSERT INTO welcomers_config(guild_id, welcome_message_enabled, " \
                  "welcome_message_channel, welcome_message_type, welcome_message_title, " \
                  "welcome_message_description, welcome_message_url) VALUES (?, ?, ?, ?, ?, ?, ?)"
            val = (guild.id, settings['default_welcome_messages_state'], 0,
                   settings['default_welcome_messages_type'], default_welcome_message_title,
                   default_welcome_message_description, default_welcome_message_url)
