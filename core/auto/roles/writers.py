...
def write_in_autoroles_marriage_standart_values(guilds) -> None:
    db = sqlite3.connect("./databases/main.sqlite")
    cursor = db.cursor()
    for guild in guilds:
        if is_guild_id_in_table("autoroles_marriage", guild.id) is False:
            sql = (
                "INSERT INTO autoroles_marriage(guild_id, autorole_id) VALUES (?, ?)"
            )
            val = (guild.id, 0)
            cursor.execute(sql, val)
            db.commit()
